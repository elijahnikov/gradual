import { VARIATION_COLORS } from "@gradual/api/utils";
import { cn } from "@gradual/ui";
import { Popover, PopoverContent, PopoverTrigger } from "@gradual/ui/popover";
import { RiCheckLine } from "@remixicon/react";
import { useState } from "react";

interface VariationColorPickerProps {
  value?: string | null;
  onChange?: (color: string) => void;
  size?: "small" | "medium" | "large";
}

export default function VariationColorPicker({
  value,
  onChange,
  size = "small",
}: VariationColorPickerProps) {
  ...
  return (
    <Popover onOpenChange={setOpen} open={open}>
      ...
      >
        {value ? (
          <span
            className={cn("rounded-[5px]", sizeClasses[size].swatch)}
          ...
          {VARIATION_COLORS.map((color) => {
            ...
            return (
              ...
              >
                ...
                />
                {isSelected && (
                  <RiCheckLine className="absolute size-4 text-white drop-shadow-sm" />
                )}
              </button>
            );
          })}
