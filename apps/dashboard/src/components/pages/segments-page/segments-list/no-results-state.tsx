...
export default function NoResultsState() {
  const [{ search }, setQueryStates] = useQueryStates(segmentsSearchParams);
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8">
      ...
      <div className="flex flex-col items-center gap-1 text-center">
        <Heading level="h2">No segments found</Heading>
        <Text className="max-w-sm text-ui-fg-muted">
          {search ? (
            <>
              No segments match "<span className="font-medium">{search}</span>".
              Try a different search term or clear the filters.
            </>
          ) : (
            "No segments match your current filters. Try adjusting your filters."
          )}
        </Text>
      </div>
